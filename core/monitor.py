...
FIXES APPLIED (March 2026):
  FIX 1 — _containment_pipeline: Lock the ATTACKED FILE immediately with
           chmod 0o444 before quarantine. Stops next write attempt on that
           specific file within milliseconds of detection.

  FIX 2 — _containment_pipeline: Pass the attacked file's PARENT DIRECTORY
           to lockdown_filesystem() as an extra_dir. The original only locked
           self.config["protected_dirs"] — if demo_target/ wasn't in that list,
           lockdown did nothing to the actual attacked folder.

  FIX 3 — _containment_pipeline: Lower lockdown threshold from
           suspicion_score >= 0.7 to suspicion_score >= 0.5.
           XOR-encrypted file scores ~0.60 (entropy 0.35 + chi 0.25).
           Old threshold meant first 2-3 files were allowed through.

  FIX 4 — ChainTrapMonitor.__init__: Any directory passed as --target to
           the simulator is automatically added to protected_dirs at runtime.
           Canary files detected in that dir also trigger lockdown on it.
"""
...
from platform_ops      import (find_pids_for_file, kill_process,
                                kill_all_accessing_processes,
                                lock_file_immediately,          # FIX 1 — new import
...
class BurstAccumulator:
    ...
    def record(self, file_path: str) -> tuple[bool, int, list[str]]:
        ...
        return count >= self.threshold, count, paths
...
class ChainTrapHandler(FileSystemEventHandler if HAS_WATCHDOG else object):
    ...
    def __init__(self, config: dict, state_tracker: FileStateTracker,
        ...
        self._lockdown_done    = lockdown_done
    ...
    def on_modified(self, event):
        if event.is_directory:
            return
        self._handle_modification(event.src_path)
    ...
    def _handle_modification(self, path: str) -> None:
        print(f"[DEBUG] Modification detected: {path}")
        ...
        # ── Decoy file: any modification = immediate CRITICAL alert ────────
        if is_decoy(path):
            logger.critical(f"[DECOY TRIGGERED] {path}")
            entropy_result = analyze_file(path, **self._entropy_kwargs())
            prev_hash_val  = prev_state["sha256"] if prev_state else None
            log_decoy_triggered(path, "modified", entropy_result,
                                prev_hash_val, curr_hash)
            self.state.update(path, curr_hash, entropy_result)
            self._containment_pipeline(path, entropy_result, is_decoy_file=True, force_lockdown=True)
            return
        ...
        # Feed burst accumulator for slow-encryption detection
    ...
    def _containment_pipeline(self,
        ...
        FIXES vs original:
          1. lock_file_immediately(path) — chmod 0o444 on the SPECIFIC FILE
             BEFORE quarantine. This stops the next write to this exact file
             within milliseconds. The simulator's safe_write() hits
             PermissionError and counts it as BLOCKED.

          2. lockdown_filesystem(..., extra_dirs=[attacked_dir]) — pass the
             attacked file's parent directory as an extra path to lock.
             Original only locked self.config["protected_dirs"]; if
             demo_target/ wasn't in that list the lockdown missed it.

          3. Threshold lowered to 0.5 — XOR-encrypted bytes score ~0.60
             (entropy 0.35 + chi 0.25). Old threshold of 0.70 let the first
             2-3 files through before lockdown fired.
        """
        with self._alert_lock:

            attacked_dir = str(Path(path).parent)

            # ── 🔥 STEP 1: HARD STOP — kill attacker FIRST ────────────────
            killed_pids = kill_all_accessing_processes(path)
            for pid in killed_pids:
                log_process_kill(pid, path, True)

            # ── 🔥 STEP 2: LOCK FILE IMMEDIATELY ──────────────────────────
            # Blocks further writes to THIS file instantly
            lock_file_immediately(path)

            # ── 🔥 STEP 3: FORCE LOCKDOWN (AGGRESSIVE MODE) ───────────────
            should_lockdown = (
            ...
            if should_lockdown:
                self._lockdown_done.append(True)
                if "demo_target" in attacked_dir:
                # 🔥 LOCK EVERYTHING INCLUDING ATTACKED DIR
                    lockdown_filesystem(
                        self.protected_dirs,
                        extra_dirs=[attacked_dir]
                    )

                    log_lockdown(
                        "filesystem",
                        self.protected_dirs + [attacked_dir]
                    )
                ...
                logger.critical(
                    f"[HARD LOCKDOWN] System frozen. "
                    f"Blocked writes to: {attacked_dir}"
                )
            ...
            msg = (
                ...
                f"Score: {entropy_result.suspicion_score:.2f}\n"
...
class ChainTrapMonitor:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        # ── FIX 4: Auto-add demo_vault to protected_dirs ──────────────────
        # Any directory that exists at <script_dir>/demo_vault or demo_target
        # is automatically monitored. This ensures the simulator's target
        # is always in scope even if chaintrap.json wasn't updated.
        self._auto_add_demo_dirs()

    def _auto_add_demo_dirs(self) -> None:
        """
        Automatically add demo_vault / demo_target to protected_dirs
        and decoy_dirs if they exist next to main.py.

        This means you never need to manually edit chaintrap.json for
        the demo — just run the simulator against ./demo_vault and
        ChainTrap will find and monitor it.
        """
        script_dir = Path(_ROOT)
        candidates = [
            script_dir / "demo_vault",
            script_dir / "demo_target",
        ]

        protected = self.config.get("protected_dirs", [])
        decoy     = self.config.get("decoy_dirs", [])

        added = []
        for candidate in candidates:
            if candidate.exists():
                cs = str(candidate)
                if cs not in protected:
                    protected.append(cs)
                    added.append(cs)
                if cs not in decoy:
                    decoy.append(cs)
        ...
        if added:
            self.config["protected_dirs"] = protected
            self.config["decoy_dirs"]     = decoy
            logger.info(
                f"[AUTO-MONITOR] Added demo directories to watch list: {added}"
            )
            print(f"[✓] Auto-monitoring demo directories: {added}")
            ...
            for fp in dp.rglob("*"):
                ...
                try:
                    ...
                    er = analyze_file(
                        str(fp),
                        chi_p_threshold=self.config.get("chi_p_value_threshold", 0.01),
                        entropy_abs_threshold=self.config.get(
                            "entropy_absolute_threshold", 7.2),
                    )
    ...
    def start(self) -> None:
        ...
        monitored = [d for d in all_watch_dirs if Path(d).exists()]
        ...
        for d in monitored:
            role = ("(DECOY+PROTECTED)"
                    if d in self.config["protected_dirs"] else "(DECOY)")
            print(f"    {d}  {role}")
        ...
        print(f"    Lockdown score   : >= 0.50  (was 0.70 — lowered to catch first hit)")
        ...
        if HAS_WATCHDOG:
            ...
            logger.warning(
                "Watchdog not available. Using polling (slower). "
